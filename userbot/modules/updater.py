...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f'**EH ADA UPDATE BARU NIH [{ac_br}]:\n\nCHANGELOG:**\n`{changelog}`'
    )
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if build.status == "failed":
            ...
            await event.edit('`Berhasil Di Update!\n'
                             'Restarting, Tunggu...`')
    else:
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit('`Berhasil Di Update!\n'
                     'Bot Sedang Restart... Tunggu beberapa Saat!`')
...
async def upstream(event):
    ...
    if ac_br != UPSTREAM_REPO_BRANCH:
        await event.edit(
            ...
            'silakan periksa ke cabang resmi apa pun`')
        return repo.__del__()
    try:
    ...
    if conf == '' and force_update is False:
        ...
        return await event.respond(
            '`Ketik command  ".update now atau .update deploy" untuk memperbarui.`')

    if force_update:
        await event.edit(
            '`Force-Syncing to latest stable userbot code, please wait...`')
    if conf == "now":
        await event.edit('`Memberbarui userbot, Silahkan Tunggu....`')
